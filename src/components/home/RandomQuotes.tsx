...
export default function RandomQuotes() {
  const [quote, setQuote] = useState('The more you research, the more you know you don\'t know.')
  const [initial, setInitial] = useState('LT20-1')
  const listQuote = [
    'LT Terbaik',
    'DX Terbaik',
    'CV Terbaik',
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      let i = Math.floor(Math.random() * listQuote.length)
      setQuote(listQuote[i])
    }, 3000)
				...
				<div className='relative max-w-7xl mx-auto pt-20 pb-12 px-4 sm:px-6 lg:px-8 lg:py-10'>
					...
					<div className='relative lg:flex lg:items-center'>
						<div className='hidden lg:block lg:flex-shrink-0'>
							<FontAwesomeIcon icon={faSmileBeam} className='h-40 w-40 rounded-full xl:h-44 xl:w-44' />
						</div>
						...
						<div className='relative lg:ml-10'>
							...
							<blockquote className='relative'>
								<div className='text-lg font-medium text-gray-600'>
									<p>Inspiring quote from Bluejacket Members</p>
								</div>
								...
								<footer className='mt-8'>
									<div className='flex'>
										<div className='flex-shrink-0 lg:hidden'>
											<FontAwesomeIcon icon={faSmileBeam} className='h-12 w-12 rounded-full' />
										</div>
