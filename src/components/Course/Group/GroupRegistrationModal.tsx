...
export default function GroupRegistrationModal({
  ...
  const createGroupNotification = async () => {
    const id = uuid()
			const course = userData.Courses.find((x) => x.ClassTransactionId == classTransactionId)
      const details = studentsData.map(x => {
        return {
          NotificationId: id,
          StudentId: x.Number,
          StudentName: x.Name
			...
			const body = {
				...
				Title: `Group Allocation Confirmation`,
				Content: `Please confirm ${course.Subject} group allocation formed by ${userData.Data.Name}`,
				ContentId: course.Subject,
				Type: `Group`,
        details: details
			}
			const result = await axios.post(`${process.env.NEXT_PUBLIC_EXTRA_URL}Notification`, body, {
        headers: {
		...
		<Transition.Root show={openRegistrationModal} as={Fragment}>
			...
			>
				<div className='flex items-center justify-center min-h-screen text-center sm:block sm:p-0'>
					...
					>
						<div className='inline-block align-bottom bg-white rounded-lg p-8 text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full sm:p-6'>
							<div>
